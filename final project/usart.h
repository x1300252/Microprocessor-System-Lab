...
#define IS_UART_READ_DONE	(USART1->ISR & USART_ISR_EOBF)	// check read data register empty

#define TX GPIO_Pin_6	// PA9
#define RX GPIO_Pin_7	// PA10

void USART_GPIO_init() {
	GPIO_Init(GPIOB, RX, GPIO_Mode_AF, GPIO_OType_PP, GPIO_PuPd_NOPULL, GPIO_Speed_Low);
	GPIO_Init(GPIOB, TX, GPIO_Mode_AF, GPIO_OType_PP, GPIO_PuPd_NOPULL, GPIO_Speed_Low);
...
void USART_Receive(char *str, int size) {
	char c;
	int pos = 0;
	while(!IS_UART_READ_DONE && pos < size) {
